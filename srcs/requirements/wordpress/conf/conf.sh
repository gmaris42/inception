...
if [ ! -f "/var/www/html/index.html" ]; then
    ...
    wp option update blogdescription "Inception by gmaris" --allow-root
    wp user create $WP_USR $WP_EMAIL --role=author --user_pass=$WP_PWD --allow-root
    curl http://loripsum.net/api/4 | wp post generate --post_content --count=2 --allow-root
    wp theme install creativeily --activate --allow-root
fi
